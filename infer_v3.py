"""
Inference script v3 — with Monte Carlo uncertainty estimation.

Architecture: GATv2Conv × 8, 10-dim physics features, 3-layer FlowEncoder,
BayesianLinear output head.

Usage:
    # Single prediction with uncertainty
    python -m Bifurcation.infer_v3 \\
        --model Models_v3/best_model_v3.pt \\
    ...
    python -m Bifurcation.infer_v3 \\
        --model Models_v3/best_model_v3.pt \\
        --geometry bifurcation_angle45_750_ascii \\
        --re-range 300 2100 100

    # Predict all cases
    python -m Bifurcation.infer_v3 --model Models_v3/best_model_v3.pt --all

    # Disable MC (deterministic, faster)
    python -m Bifurcation.infer_v3 ... --mc-samples 1
"""
...
from Bifurcation.dataset_v3 import denormalize_wss_v3
...
def load_model_v3(
    ...
    """Load a v3 checkpoint. Returns (model, norm_stats)."""
    ...
    if norm_stats is None:
        raise RuntimeError("Checkpoint has no norm_stats — retrain with train_v3.py")
...
def _build_inference_graph(
    ...
    """
    Load mesh + compute v3 features (10-dim physics, no LPE) → normalised PyG Data.
    ...
    """
    geo_path = str(config_v3.get_geometry_path(geometry_folder))
    angle, _ = config_v3.parse_geometry_folder(geometry_folder)
    re_case  = f"Re{int(re_number)}"

    (wall_centres, _wss_gt, edge_index, edge_attr,
     _re, boundary, surface_mesh_data) = load_sample(
        geo_path, re_case, mesh_re=config_v3.mesh_re, return_surface_mesh=True,
    )
    all_points, wall_face_vertices, _ = surface_mesh_data

    poly_path = (config_v3.get_geometry_path(geometry_folder)
                 / config_v3.mesh_re / "constant" / "polyMesh")
    all_faces = parse_openfoam_faces(str(poly_path / "faces"))

    # 10-dim physics features (same as v2; no LPE for inference)
    ...
    # Normalise — norm_stats may have 18-dim x_mean (from LPE-trained cache);
    # slice to first 10 which correspond to the physics features.
...
def predict_single_v3(
    ...
    wss_std  : (N, 3)  std across MC passes (physical units)
    """
    device = device or next(model.parameters()).device

    coords, data = _build_inference_graph(geometry_folder, re_number, norm_stats, device)
...
def _print_branch_summary(coords: np.ndarray, wss_mean: np.ndarray, wss_std: np.ndarray):
    z     = coords[:, 2]
    z_med = np.median(z)
    for label, mask in [("Parent  (lower Z)", z < z_med),
                         ("Daughter(upper Z)", z >= z_med)]:
        if mask.any():
            mag  = np.linalg.norm(wss_mean[mask], axis=1)
            smag = np.linalg.norm(wss_std[mask],  axis=1)
            print(f"  {label}: n={mask.sum():6d}  "
                  f"mean_mag={mag.mean():.4e}  mean_std={smag.mean():.4e}")
...
def main():
    parser = argparse.ArgumentParser(description="Inference v3 — MC uncertainty")
    parser.add_argument("--model",      type=str,
                        default=str(config_v3.models_dir / "best_model_v3.pt"))
    ...
    parser.add_argument("--paraview",   action="store_true",
                        help="Export ParaView case with uncertainty field")
    ...
    out_root = Path(args.output) if args.output else config_v3.predictions_dir
    out_root.mkdir(parents=True, exist_ok=True)

    print(f"Loading model from {args.model} …")
    model, norm_stats = load_model_v3(args.model, device)
    ...
    for geo, re_val in pairs:
        ...
        coords, wss_mean, wss_std = predict_single_v3(
        ...
        # Save CSV: x,y,z, wss_x/y/z, wss_magnitude, std_x/y/z, std_magnitude
        mag_mean = np.linalg.norm(wss_mean, axis=1)
        mag_std  = np.linalg.norm(wss_std,  axis=1)
        csv_path = case_dir / "predicted_wss_v3.csv"
        ...
        if args.paraview:
            geo_src = config_v3.get_geometry_path(geo) / config_v3.mesh_re
            ...
            _write_openfoam_wss(str(ts_dir / "wallShearStress"), wss_mean, boundary)
            _write_scalar_openfoam(str(ts_dir / "wss_uncertainty"), mag_std, boundary)
            (case_dir / "prediction_v3.foam").touch()
